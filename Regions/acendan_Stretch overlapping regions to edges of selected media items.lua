-- @description Stretch overlapping regions to edges of selected media items
-- @author Aaron Cendan
-- @version 1.0
-- @metapackage
-- @provides
--   [main] . > acendan_Stretch overlapping regions to edges of selected media items.lua
  ...
  if num_regions > 0 then
    ...
    if num_sel_items > 0 then
      ...
      if start_time ~= end_time then
        ...
        reaper.MB("The selected items have no length! How is this even possible?","Stretch Region Edges", 0)
      end
    else
      reaper.MB("No items selected!","Stretch Region Edges", 0)
    end
  else
