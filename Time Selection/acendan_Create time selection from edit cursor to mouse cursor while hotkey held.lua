...
-- @metapackage
-- @provides
--   [main] . > acendan_Create time selection from edit cursor to mouse cursor while hotkey held.lua
...
function UserConfig()
  ...
  key = keys.S    -- Replace with your preferred key. For a list of all available keys, refer to the 'keys' table below. 
  ctrl = false    -- Toggle for ctrl key modifier   - true/false
  shift = false   -- Toggle for shift key modifier  - true/false
  alt = true     -- Toggle for alt key modifier    - true/false
...
local run_previously = reaper.HasExtState( "acendan_Create time selection", "first_time" )  
if run_previously then
  ...
  reaper.SetExtState( "acendan_Create time selection", "first_time", "https://youtu.be/dQw4w9WgXcQ", true )
